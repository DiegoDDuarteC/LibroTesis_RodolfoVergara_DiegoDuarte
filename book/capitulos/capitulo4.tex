...
\begin{itemize}
    \item \textbf{Umbral mínimo ($\text{BFR}_F$ = $\text{BFR}_{F_{\text{MIN}}}$):} Indica el inicio de fragmentación apreciable
    \item \textbf{Umbral crítico ($\text{BFR}_F$ = $\text{BFR}_{F_{\text{MAX}}}$):} Señala estados de alta fragmentación que incrementan significativamente la probabilidad de bloqueo
\end{itemize}

La lógica de actuación se define como:

\begin{enumerate}
    \item \textbf{Sin desfragmentación:} Si la predicción del primer modelo indica que el $\text{BFR}_F$ no superará el umbral mínimo ($\text{BFR}_F$ $< 0.20$)
    
    En este escenario, el sistema determina que la red mantendrá un estado saludable con fragmentación insignificante durante el horizonte predicho. Dado que por debajo de este umbral los bloqueos son mínimos o nulos, no se justifica el costo operacional de ejecutar una desfragmentación. El sistema suprime cualquier intervención de reordenamiento y programa la siguiente consulta al modelo predictivo para 1,500 unidades de tiempo después. Esta decisión maximiza la eficiencia operacional al evitar reconfiguraciones innecesarias cuando los recursos espectrales se encuentran adecuadamente distribuidos.
    
    \item \textbf{Desfragmentación de baja frecuencia:} Si el primer modelo predice que se superará el umbral mínimo ($\text{BFR}_F$ $\geq 0.20$), pero el segundo modelo indica que no se alcanzará el umbral crítico ($0.20 \leq$ $\text{BFR}_F$ $< 0.46$)
    
    Esta condición indica que la red entrará en un rango donde comenzarán a aparecer bloqueos debido a fragmentación moderada, aunque sin alcanzar niveles críticos. La estrategia preventiva consiste en ejecutar inmediatamente un proceso de desfragmentación para reorganizar el espectro y minimizar los bloqueos emergentes antes de que la situación se agrave. Tras esta intervención, el sistema establece un intervalo de monitoreo de 1,500 unidades de tiempo antes de realizar la siguiente consulta predictiva. Este intervalo relativamente largo es apropiado porque en este rango de fragmentación la red aún dispone de recursos razonables y la degradación evoluciona de manera gradual, permitiendo espaciar las intervenciones sin comprometer significativamente el desempeño.
    
    \item \textbf{Desfragmentación de alta frecuencia:} Si ambos modelos coinciden en que el $\text{BFR}_F$ predicho superará el umbral crítico ($\text{BFR}_F$ $\geq 0.46$)
    
    En este caso crítico, el sistema anticipa que la red alcanzará niveles de fragmentación donde la cantidad de bloqueos se incrementa considerablemente. La escasez severa de bloques espectrales contiguos compromete significativamente la capacidad de la red para aceptar nuevas conexiones. La respuesta consiste en ejecutar inmediatamente una desfragmentación reactiva para mitigar el impacto de esta fragmentación crítica anticipada. Sin embargo, dada la severidad de la situación y la velocidad con que evoluciona la fragmentación en estados de alta congestión, el sistema reduce el intervalo de monitoreo a 800 unidades de tiempo. Este intervalo más corto permite al sistema mantenerse vigilante y responder ágilmente mediante intervenciones frecuentes, asegurando que la fragmentación crítica se gestione de manera intensiva para contener el aumento de bloqueos durante períodos de alta demanda.
\end{enumerate}

Esta aproximación permite actuar de manera preventiva antes de que la red alcance estados críticos \cite{comellas2018periodic}, optimizando el balance entre los costos operacionales de la desfragmentación y el mantenimiento de niveles aceptables de fragmentación. La estrategia de tres niveles con intervalos adaptativos (sin intervención cuando $\text{BFR}_F < 0.20$, intervalo de 1,500 unidades cuando $0.20 \leq \text{BFR}_F < 0.46$, e intervalo de 800 unidades cuando $\text{BFR}_F \geq 0.46$) constituye el mecanismo fundamental mediante el cual el sistema traduce las predicciones de los modelos de Machine Learning en acciones concretas de gestión de red. Esta arquitectura dual de predictores maximiza la eficiencia al intensificar intervenciones solo cuando es verdaderamente necesario y suprimiéndolas cuando la red opera en condiciones saludables.
...
\textbf{Nota:} Todas las métricas descritas a continuación se capturan en el instante temporal $t$ de la simulación.

\begin{itemize}
    \item \textbf{Tiempo de Simulación ($t$)}: Instante temporal en el cual se captura el estado de la red. Permite identificar patrones temporales y correlaciones con las fases operativas del sistema.

    \item \textbf{Bloqueos ($B_t$)}: Número de demandas bloqueadas en el instante actual de la simulación. Esta métrica refleja la incapacidad de la red para satisfacer solicitudes de conexión debido a insuficiencia de recursos espectrales disponibles.

    \item \textbf{Demandas Activas ($D_{active}$)}: Cantidad de conexiones activas establecidas en la red. Indica el nivel de ocupación de recursos y se utiliza como factor espacial en la métrica ASFR3D.

    \item \textbf{Tasa de Bloqueo Instantánea ($TBI_t$)}: Ratio entre demandas bloqueadas y demandas procesadas:
    \begin{equation}
        TBI_t = \frac{B_t}{D_{processed,t}}
    \end{equation}
    donde $D_{processed,t}$ es el número total de demandas procesadas (aceptadas y bloqueadas) en el instante actual. Esta métrica proporciona una medida instantánea del rendimiento de la red.
    ...
    \end{equation}
    donde \textit{MaxBlock()} es el tamaño del mayor bloque de \textit{Frequency Slot} (FS) libres y \(S^{free}\) es la suma total de FS libres en el core.
    ...
    El SC de la red se calcula como el promedio aritmético simple sobre todos los cores de todos los enlaces:
    \begin{equation}
        SC_{red} = \frac{1}{|E| \times C} \sum_{i=1}^{|E|} \sum_{j=1}^{C} SC_{core_{i,j}}
    \end{equation}
    donde \(|E|\) es el número total de enlaces en la red y \(C\) es el número de cores por enlace.
    ...
    El GM de la red se calcula como el promedio aritmético simple sobre todos los cores de todos los enlaces:
    \begin{equation}
        GM_{red} = \frac{1}{|E| \times C} \sum_{i=1}^{|E|} \sum_{j=1}^{C} GM_{core_{i,j}}
    \end{equation}
    donde \(|E|\) es el número total de enlaces en la red y \(C\) es el número de cores por enlace.
    ...
    El ASFR3D de la red se calcula como el promedio aritmético simple sobre todos los cores de todos los enlaces:
    \begin{equation}
        ASFR3D_{red} = \frac{1}{|E| \times C} \sum_{i=1}^{|E|} \sum_{j=1}^{C} ASFR3D_{core_{i,j}}
    \end{equation}
    donde \(|E|\) es el número total de enlaces en la red y \(C\) es el número de cores por enlace.
...
Se utilizó un simulador de redes MC-EON \cite{davalos2019spectrum} para generar el conjunto de datos de entrenamiento. La topología empleada fue USNET, representativa de redes ópticas de producción.
...
\subsection{Modelo para Umbral Mínimo ($\text{BFR}_F$ = 0.20)}
...
\subsection{Modelo para Umbral Crítico ($\text{BFR}_F$ = 0.46)}
...
\subsection{Metodología de Selección del Horizonte}

El horizonte temporal de predicción fue seleccionado mediante análisis 
de correlación temporal sobre las 399,000 muestras del dataset sintético. 
Para cada muestra $i$, se calculó:

\begin{itemize}
    \item Autocorrelación entre $BFR_i$ y $BFR_{i+h}$ para diferentes 
          horizontes $h \in \{500, 1000, 1500, 2000\}$
    \item Persistencia como proporción de casos donde 
          $|BFR_{i+h} - BFR_i| < 0.10$
    \item Volatilidad como desviación estándar de $BFR_{i+h} - BFR_i$
\end{itemize}

El horizonte $h=1000$ presentó el mejor balance entre predictibilidad 
y anticipación suficiente.
...
\subsection{Resultados del Modelo de Umbral Crítico ($\text{BFR}_F$ = 0.46)}
...
\begin{table}[htbp]
\centering
\caption{Resultados del modelo Gradient Boosting para umbral crítico ($\text{BFR}_F$ = 0.46)}
...
\subsection{Resultados del Modelo de Umbral Mínimo ($\text{BFR}_F$ = 0.20)}
...
\begin{table}[htbp]
\centering
\caption{Resultados del modelo Gradient Boosting para umbral mínimo ($\text{BFR}_F$ = 0.20)}
\label{tab:resultados_020}
\begin{tabular}{lcc}
\hline
\textbf{Métrica} & \textbf{Valor} & \textbf{Interpretación} \\
\hline
Precisión & 0.970 & 97.0\% de alertas críticas son correctas \\
Recall & 0.963 & 96.3\% de situaciones críticas detectadas \\
F1-Score & 0.966 & Balance muy robusto precisión-cobertura \\
AUC-ROC & 0.992 & Excelente capacidad discriminativa \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    \item Precisión $\geq$ 0.95: Cumple ampliamente
    \item F1-Score $\geq$ 0.95: Cumple ampliamente
\end{itemize}
