...
Esta aproximación permite actuar de manera preventiva antes de que la red alcance estados críticos, optimizando el balance entre los costos operacionales de la desfragmentación y el mantenimiento de niveles aceptables de fragmentación.

\section{Características Utilizadas}

En el contexto de \textit{Machine Learning}, las características son los parámetros de entrada que alimentan el modelo predictivo. Para este trabajo se seleccionaron métricas relacionadas con la fragmentación, utilización de recursos y comportamiento específico de redes MC-EON:

\begin{itemize}
    \item \textbf{Bandwidth Fragmentation Ratio (BFR)}\cite{zhang2013bandwidth}: Representa el índice de fragmentación de los recursos de la red. El BFR de un core en un enlace se define como:
    \begin{equation}
    ...
    \item \textbf{Entropía de Shannon (SHF)}\cite{wright2015minimum}: Mide la distribución de bloques libres en el espectro. Para un core:
    \begin{equation}
    ...
    \item \textbf{Compacidad del Espectro (SC)}: Evalúa qué tan compacto está el uso del espectro considerando la dispersión de FS ocupados y la cantidad de gaps intermedios. Para un core:
    \begin{equation}
    ...
    \item \textbf{Golden Metric (GM)}: Métrica avanzada que evalúa la fragmentación considerando rangos de tamaño de demandas esperadas. Dados dos parámetros \(n_1\) y \(n_2\) que representan tamaños típicos de demanda, el GM para un core se calcula como:
    \begin{equation}
        GM_{core} = \frac{a}{|b|}
    \end{equation}
    ...
    \item \textbf{Available Spectrum Fragmentation Ratio 3D (ASFR3D)}: Considera la fragmentación espacial en redes multi-core, evaluando bloques pequeños de difícil utilización:
    \begin{equation}
    ...
    \item \textbf{Utilización Diferencial (UD)}: Mide el desbalance en la utilización entre cores y enlaces:
    \begin{equation}
...
Los componentes de esta función tienen roles específicos:
\begin{itemize}
    \item \(\sin(3\pi t)\): Ciclo principal de carga
    \item Componentes de alta frecuencia: Microfluctuaciones que simulan perturbaciones menores
    \item \(-0.25\,\cos(5\pi t)\): Introduce asimetrías y prolonga fases de baja carga
    \item \(0.15\,\sin(\pi t)\): Tendencia global de modulación
\end{itemize}
...
\begin{figure}[H]
    ...
    \label{fig:SIMULACION_COMPONENTES_ARMONICAS}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{capitulos/img/SIMULACION_MODELO_MATEMATICO_COMPLETO.png}
    \caption{Modelo matemático completo de evolución de carga}
    \label{fig:SIMULACION_MODELO_COMPLETO}
\end{figure}

\subsection{Niveles Discretos de Carga}

Los valores continuos de \(C(t)\), que oscilan aproximadamente en \([-1.8, 1.8]\), se mapean a doce niveles discretos de carga. Cada nivel se asocia con un valor en Erlangs y una denominación cualitativa del estado de la red (desde "valle profundo" hasta "pico máximo"). La Tabla~\ref{fig:SIMULACION_TABLA1_NIVELES DE CARGA} presenta estos niveles.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{capitulos/img/SIMULACION_TABLA1_NIVELES DE CARGA.png}
    \caption{Niveles de carga simulados}
...
\begin{figure}[H]
    ...
    \label{fig:SIMULACION_FRECUENCIA}
\end{figure}

\subsection{Identificación de Fases Operativas}

Para cada instante de simulación se identifica la fase operativa del sistema basándose en la tendencia local de la carga (derivada aproximada por diferencias finitas):

\begin{itemize}
    \item \textbf{Ascenso:} El valor de Erlangs aumenta progresivamente
    \item \textbf{Pico:} Máximo local de carga
    \item \textbf{Descenso:} La carga disminuye gradualmente
    \item \textbf{Valle:} Mínimo local de carga
    \item \textbf{Estable:} Variaciones menores entre instantes consecutivos
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Distribución de fases operativas}
    \label{fig:SIMULACION_FASES_OPERATIVAS}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{capitulos/img/SIMULACION_TABLA2_MUESTRA_DE_DATOS_TEMPORALES.png}
    \caption{Muestra de datos temporales generados}
    \label{fig:SIMULACION_MUESTRA_DATOS}
\end{figure}

\section{Herramientas de Implementación}

Se utilizó el ecosistema \textbf{Scikit-learn}\cite{scikit-learn} en Python para todo el proceso de Machine Learning. Esta biblioteca proporciona algoritmos de aprendizaje supervisado optimizados junto con herramientas para preprocesamiento, validación y evaluación de modelos.
...
Ambos modelos utilizan \textbf{Gradient Boosting Classifier} con la siguiente configuración optimizada:
...
Gradient Boosting construye árboles de decisión de forma secuencial, donde cada árbol corrige los errores del anterior mediante minimización de la función de pérdida (log loss) por descenso de gradiente. La tasa de aprendizaje baja combinada con 150 estimadores permite un aprendizaje gradual y robusto, reduciendo el riesgo de sobreajuste.
...
Estos indicadores confirman que el horizonte de 1000 demandas presenta estabilidad temporal suficiente para entrenar modelos efectivos de Machine Learning.
...
El modelo alcanzó la clasificación de \textbf{VIABLE}, superando ampliamente los criterios establecidos:
\begin{itemize}
...
\textbf{Métrica} & \textbf{Valor} & \textbf{Interpretación} \\
\hline
Precisión & >0.99 & Mínimos falsos positivos \\
Recall & >0.99 & Muy alta cobertura de detección \\
F1-Score & >0.99 & Balance óptimo \\
...
El modelo alcanzó métricas excepcionalmente altas, clasificándose como \textbf{VIABLE} con desempeño superior:
\begin{itemize}
...
Si bien durante el proceso se evaluaron tres algoritmos (Random Forest, Gradient Boosting y Regresión Logística), se seleccionó Gradient Boosting como modelo definitivo para ambos umbrales por las siguientes razones:
...
Cada servicio proporciona los siguientes endpoints HTTP:

\begin{itemize}
    \item \texttt{POST /predict}: Recibe métricas actuales de red y retorna predicción con probabilidad y recomendación de desfragmentación
    \item \texttt{GET /health}: Verifica estado operativo del servicio y modelo cargado
    \item \texttt{GET /info}: Retorna información del modelo (tipo, métricas de entrenamiento, features requeridas, umbrales)
    \item \texttt{POST /reload}: Recarga el modelo desde archivos sin reiniciar el servicio
\end{itemize}
...
Las solicitudes al endpoint \texttt{/predict} requieren un JSON con las 11 características del modelo:

\begin{verbatim}
{
  "Demandas_Procesadas": 15420,
  "Bloqueos": 234,
  "Demandas_Activas": 1850,
  "Tasa_Bloqueo_Instantaneo": 0.0152,
  "Compactacion": 0.342,
  "Fragmentacion": 0.418,
  "Entropia": 2.145,
  "Golden_Metric": 1.876,
  "ASFR_3D": 0.267,
  "Utilizacion_Diferencial": 0.183,
  "Prob_Bloqueo_Hacia_Atras": 0.0089
}
\end{verbatim}

La respuesta incluye la predicción procesada:
...
\begin{verbatim}
{
  ...
  "resultado": {
    "horizonte": 1000,
    "probabilidad_critica": 0.752,
    "debe_desfragmentar": true,
    "umbral_usado": 0.70,
    "confianza": "alta",
    "viabilidad": "VIABLE",
    "precision_esperada": 0.957,
    "f1_esperado": 0.900,
...
\subsection{Persistencia y Reproducibilidad}

Cada modelo entrenado se persiste en tres archivos:

\begin{itemize}
    \item \texttt{modelo\_bfr\_t1000.pkl}: Modelo Gradient Boosting serializado con Joblib
    \item \texttt{scaler\_bfr\_t1000.pkl}: Escalador RobustScaler ajustado
    \item \texttt{config\_bfr\_t1000.json}: Configuración completa (features, umbrales, métricas, timestamp)
\end{itemize}

Esta estructura garantiza reproducibilidad exacta de las predicciones y facilita la auditoría y versionado de modelos en ambientes de producción.