...
\begin{enumerate}
    \item \textbf{Sin desfragmentación:} Si la predicción indica que el $\text{BFR}_F$ no superará el umbral mínimo ($\text{BFR}_F$ $< 0.20$)
    \item \textbf{Desfragmentación de baja frecuencia:} Si el $\text{BFR}_F$ predicho supera el umbral mínimo pero no alcanza el umbral crítico ($0.20 \leq$ $\text{BFR}_F$ $< 0.46$)
    \item \textbf{Desfragmentación de alta frecuencia:} Si el $\text{BFR}_F$ predicho supera el umbral crítico ($\text{BFR}_F$ $\geq 0.46$)
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Bloqueos en Tiempo $t$ ($B_t$)}: Número de demandas bloqueadas en el instante actual de la simulación. Esta métrica refleja la incapacidad de la red para satisfacer solicitudes de conexión debido a insuficiencia de recursos espectrales disponibles.

    \item \textbf{Demandas Activas en Tiempo $t$ ($D_{active}$)}: Cantidad de conexiones activas establecidas en la red en el instante $t$. Indica el nivel de ocupación de recursos y se utiliza como factor espacial en la métrica ASFR3D.

    \item \textbf{Tasa de Bloqueo Instantánea ($TBI_t$)}: Ratio entre demandas bloqueadas y demandas procesadas en el instante $t$:
    \begin{equation}
        TBI_t = \frac{B_t}{D_{processed,t}}
    \end{equation}
    donde $D_{processed,t}$ es el número total de demandas procesadas (aceptadas y bloqueadas) hasta el instante $t$. Esta métrica proporciona una medida instantánea del rendimiento de la red.
